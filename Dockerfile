...
RUN wget https://golang.org/doc/install?download=go1.8.7.linux-arm64.tar.gz -qO /usr/local/bin/golang/go1.8.tgz
RUN ls -l
RUN tar -C /usr/local/bin/golang/ -xzf go1.8.tgz;
